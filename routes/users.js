...
router.post('/forgot/:user', (req, res) => {  
  ...
  // const password = req.body.password;
  modelUsers.readUser(user, function(error, user) {
    if (error) throw error;
    if (!user) {
      res.status(401).json({ message: 'Tên đăng nhập hoặc email không đúng' });
    } else {
      if(user.email===email){
        const tempPassword = generateTempPassword(); // Tạo mật khẩu tạm thời
        req.session.tempPassword = tempPassword; // Lưu vào session
        
        res.json({ message: 'Mật khẩu của bạn là', password: tempPassword }); // Trả về mật khẩu tạm thời cho người dùng
        const salt = bcrypt.genSaltSync(10);
        // Hash password với salt
        const hashedPassword = bcrypt.hashSync(req.session.tempPassword, salt);
        db.query('UPDATE users SET password=? WHERE username=?',[hashedPassword, user.username], (err,p)=>{
          if(err) throw err;
          console.log(p);  
